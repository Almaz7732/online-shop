...
@section('title', $selectedCategory ? $selectedCategory->name . ' - ' . config('app.name', 'Example') : 'Продукты - ' . config('app.name', 'Example'))
                ...
                <ul class="breadcrumb-nav">
                    ...
                    @if($selectedCategory)
                        @if($selectedCategory->parent)
                            <li><a href="{{ route('shop.products.category', $selectedCategory->parent->slug) }}">{{ $selectedCategory->parent->name }}</a></li>
                        @endif
            ...
            <div class="row mb-4">
                <div class="col-12">
                    <div class="search-info alert alert-light">
                        <h5>Результаты поиска для: "{{ request('search') }}"</h5>
                        <p class="mb-0">Найдено товаров: {{ $products->total() }}</p>
                        @if($products->total() == 0)
                            <a href="{{ route('shop.products') }}" class="btn btn-primary btn-sm mt-2">Показать все товары</a>
                        @endif
                    </div>
                </div>
            </div>
                        ...
                        <div class="categories-widget">
                            <ul class="category-list">
                                ...
                                @foreach($categories as $category)
                                    ...
                                    @endphp
                                    <li class="has-children {{ $isCategoryActive ? 'active' : '' }}" data-category-id="{{ $category->id }}">
                                        <div class="category-item">
                                            <a href="{{ route('shop.products.category', $category->slug) }}" class="category-link">
                                                {{ $category->name }}
                                                <span class="count">({{ $category->total_products_count }})</span>
                                            </a>
                                            @if($category->children->count() > 0)
                                                <span class="category-toggle" data-category="{{ $category->id }}">
                                        ...
                                        @if($category->children->count() > 0)
                                            <ul class="sub-categories">
                                                @foreach($category->children as $child)
                                                    ...
                                                    @endphp
                                                    <li class="has-children {{ $isChildActive ? 'active' : '' }}">
                                                        <div class="category-item">
                                                            <a class="category-link" href="{{ route('shop.products.category', $child->slug) }}">
                                                                {{ $child->name }}
                                                                <span class="count">({{ $child->total_products_count }})</span>
                                                            </a>
                                                            @if($child->children->count() > 0)
                                                                <span class="category-toggle" data-category="{{ $child->id }}">
                                                                    <i class="lni lni-plus"></i>
                                                                </span>
                                                            @endif
                                                        </div>
                                                        @if($child->children->count() > 0)
                                                            <ul class="sub-categories level-3">
                                                                @foreach($child->children as $grandchild)
                                                                    <li class="{{ ($selectedCategory && $selectedCategory->id == $grandchild->id) ? 'active' : '' }}">
                                                                        <a class="category-link" href="{{ route('shop.products.category', $grandchild->slug) }}">
                                                                            {{ $grandchild->name }}
                                                                            <span class="count">({{ $grandchild->total_products_count }})</span>
                                                                        </a>
                                                                    </li>
                                                                @endforeach
                                                            </ul>
            ...
            <div class="col-lg-9 col-12">
                <div class="product-grids-head">
                        ...
                        <div class="row align-items-center">
                            <div class="col-lg-7 col-md-8 col-12">
                                <div class="product-sorting">
                                    ...
                                    <p class="product-show">Показано {{ $products->firstItem() ?? 0 }}–{{ $products->lastItem() ?? 0 }} из {{ $products->total() }} результатов</p>
                                </div>
                            </div>
                            <div class="col-lg-5 col-md-4 col-12">
                                <nav>
                                    <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                        <button class="nav-link {{ $view == 'grid' ? 'active' : '' }}" id="nav-grid-tab" data-bs-toggle="tab" data-bs-target="#nav-grid" type="button" role="tab" aria-controls="nav-grid" aria-selected="{{ $view == 'grid' ? 'true' : 'false' }}" data-view="grid">
                                            <i class="lni lni-grid-alt"></i>
                                        </button>
                                        <button class="nav-link {{ $view == 'list' ? 'active' : '' }}" id="nav-list-tab" data-bs-toggle="tab" data-bs-target="#nav-list" type="button" role="tab" aria-controls="nav-list" aria-selected="{{ $view == 'list' ? 'true' : 'false' }}" data-view="list">
                            ...
                            <div class="product-sorting-menu">
                                <div class="sorting-list">
                                    <select class="form-select" id="sort-select">
                                        <option value="latest" {{ $sort == 'latest' ? 'selected' : '' }}>Сортировка по новизне</option>
                                        <option value="price_low" {{ $sort == 'price_low' ? 'selected' : '' }}>Цена: по возрастанию</option>
                                        <option value="price_high" {{ $sort == 'price_high' ? 'selected' : '' }}>Цена: по убыванию</option>
                                        <option value="name_az" {{ $sort == 'name_az' ? 'selected' : '' }}>Название: А-Я</option>
                                        <option value="name_za" {{ $sort == 'name_za' ? 'selected' : '' }}>Название: Я-А</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="tab-content" id="nav-tabContent">
                        <!-- Grid View -->
                        <div class="tab-pane fade {{ $view == 'grid' ? 'show active' : '' }}" id="nav-grid" role="tabpanel" aria-labelledby="nav-grid-tab">
                            <div class="row">
...
{{--                                                        @endif--}}
{{--                                                    @endfor--}}
{{--                                                    <li><span>{{ rand(10, 99) }} отзывов</span></li>--}}
{{--                                                </ul>--}}
                                                <div class="price">
                                ...
                                @empty
                                    @include('clients.shop.partials.empty-products')
                                @endforelse
                            </div>
                        </div>

                        <!-- List View -->
                        <div class="tab-pane fade {{ $view == 'list' ? 'show active' : '' }}" id="nav-list" role="tabpanel" aria-labelledby="nav-list-tab">
                            <div class="row">
                                @forelse($products as $product)
                                    <div class="col-12">
                                                ...
                                                <div class="col-lg-3 col-md-3 col-12">
                                                    <div class="product-price-actions">
                                                        ...
                                                        <div class="action-buttons">
                                                            <a href="javascript:void(0)" class="btn btn-primary add-to-cart" data-product-id="{{ $product->id }}" title="Добавить в корзину">
                                                                <i class="lni lni-cart"></i> В корзину
                                                            </a>
                                                            <a href="{{ route('shop.product-details', $product->slug) }}" class="btn btn-secondary btn-sm" title="Подробности">
                                                                Подробнее
                                                            </a>
                                                        </div>
                                                    </div>
