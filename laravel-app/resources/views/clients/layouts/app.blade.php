...
<head>
    ...
    @else
        <title>@yield('title',  config('app.name', 'Example') . ' - Магазин техники и гаджетов')</title>
        <meta name="description" content="@yield('meta_description', 'Поставка техники и гаджетов в Кыргызстан')" />
    @endif
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <link rel="shortcut icon" type="image/x-icon" href="{{ asset('build/images/site-logo-x-icon.jpg') }}" />

    <!-- ========================= CSS here ========================= -->
    @vite(['resources/clients/css/app.css'])
    @stack('styles')
...
<body>
    ...
    <!-- ========================= JS here ========================= -->
    @vite(['resources/clients/js/app.js'])

    <!-- ShopGrids JS Files -->
    <script src="{{ asset('build/clients/js/bootstrap.min.js') }}"></script>
    <script src="{{ asset('build/clients/js/tiny-slider.js') }}"></script>
    <script src="{{ asset('build/clients/js/glightbox.min.js') }}"></script>
    <script src="{{ asset('build/clients/js/main.js') }}"></script>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function initializeSearch(input, results, loading, items, footer) {
                ...
                input.addEventListener('keydown', function(event) {
                    if (event.key === 'Enter') {
                        ...
                        const query = this.value.trim();
                        if (query.length >= 2) {
                            window.location.href = `{{ route('shop.products') }}?search=${encodeURIComponent(query)}`;
                        }
                    }
                });
            }
            ...
            function performSearch(query, loading, items, footer) {
                fetch(`{{ route('shop.search-suggestions') }}?q=${encodeURIComponent(query)}`)
